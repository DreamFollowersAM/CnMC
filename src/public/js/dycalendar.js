class Calendar {
    ...
    getTemplate() {
        let template = `
            <div class="calendar__header fade-scroll">
    ...
    showCells() {
        ...
        for (let i = 0; i < this.cells.length; i++) {
            ...
            if(i == 6 || i == 13 || i == 20 || i == 27 || i == 34){
                templateCells += `
                <span class="gray grid__cell grid__cell--gd ${disabledClass}" data-cell-id="${i}">
                    ${this.cells[i].date.date()}
                </span>`;
            }else if(cntr > 9){
                templateCells += `
                <span class="red grid__cell grid__cell--gd ${disabledClass}" data-cell-id="${i}">
                    ${this.cells[i].date.date()}
                </span>`;
            }else if(cntr < 10 && cntr > 4){
                templateCells += `
                <span class="orange grid__cell grid__cell--gd ${disabledClass}" data-cell-id="${i}">
                    ${this.cells[i].date.date()}
                </span>`;
            }else if(cntr < 5){
                templateCells += `
                <span class="green grid__cell grid__cell--gd ${disabledClass}" data-cell-id="${i}">
                    ${this.cells[i].date.date()}
                </span>`;
            }else{
                templateCells += `
            ...
            console.log(this.cells[i].isInCurrentMonth);
        }
